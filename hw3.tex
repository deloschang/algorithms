...
\begin{document}
  \begin{enumerate}
    ...
    {\bf Time Complexity:}
    If the algorithm discovers that the graph is acyclic (no back edges), there are at most $|V| - 1$ edges.
    Together with DFS' time complexity of $O(|V| + |E|)$, this algorithm's time complexity is $O(V)$. 

    If the algorithm discovers that the graph is cyclic, it has found a back edge. The algorithm stops before 
    checking at most $|V|$ edges and vertices. Therefore, in this case, the algorithm's time commplexity is also $O(V)$. 
    ...
    \item
      ...
      a) $a = 16, b = 4, f(n) = n^2$. $n^{\log_b a} = n^{\log_4 16} = n^2$. 
      ...
      b) $a = 7, b = 3, f(n) = n^2$. $n^{\log_b a} = n^{\log_3 7}$. 

      Thus, $f(n) = \Omega(n^{\log_3 7 + \epsilon})$ for some constant $\epsilon>0$ holds because of:
      $\log_3 7 \approx 1.77$. For instance, consider $\epsilon=0.1$.

      From the proof in \#10, because $f(n) = \Omega(n^{\log_3 7 + \epsilon})$ and $f(n) = n^k$ where $k=2$,
      the regularity condition must hold. For example, consider when $c = 7/9$. 
      ...
      Hence, Case 3 (from CLRS) of the Master Method applies and $T(n) = \Theta(f(n)) = \Theta(n^2)$.
